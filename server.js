...
function broadcastToPair(pairId, event, data, excludeWs = null) {
...
app.post('/api/diary/save', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'diary:updated', { date, role: user.role, text: content, updatedAt: Date.now() });
...
app.post('/api/voucher/create', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'voucher:created', voucher);
...
app.post('/api/voucher/update', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'voucher:updated', { id: voucherId, status });
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/wish/create', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'wish:created', wish);
...
app.post('/api/wish/toggle', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'wish:toggled', { id: wishId, completed: !!rows[0]?.completed });
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/wish/delete', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'wish:deleted', { id: wishId });
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/countdown/create', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'countdown:created', countdown);
...
app.post('/api/mood/update', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'mood:updated', { role: user.role, mood });
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/anniversary/update', async (req, res) => {
  try {
    ...
    broadcastToPair(user.pair_id, 'anniversary:updated', { date });
    res.json({ success: true });
  } catch (err) {
