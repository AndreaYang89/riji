...
export default function CoupleDiaryApp() {
  ...
  const handleFabClick = () => {
    if (activeTab === 'vouchers') setShowVoucherModal(true);
  ...
  if (appState === 'unpaired' || appState === 'waiting') {
    return <PairingScreen appState={appState} inviteCode={inviteCode} onCreatePair={handleCreatePair} onJoinPair={handleJoinPair} theme={theme} />;
  }
    ...
    <div style={{ height: '100dvh', width: '100%', display: 'flex', justifyContent: 'center', fontFamily: 'system-ui,sans-serif', overflow: 'hidden', background: theme.appBg }}>
      <div style={{ width: '100%', maxWidth: 448, height: '100%', position: 'relative', display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.4)', boxShadow: '0 25px 50px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ padding: '24px 24px 8px', flexShrink: 0 }}>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: '#334155', margin: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 120px' }}>
          {activeTab === 'calendar' && <DashboardView diaryData={diaryData} myRole={myRole} moods={moods} countdowns={countdowns} anniversary={anniversary} theme={theme} />}
          {activeTab === 'wishlist' && <WishlistView wishlist={wishlist} myRole={myRole} theme={theme} />}
          {activeTab === 'vouchers' && <VoucherView vouchers={vouchers} myRole={myRole} theme={theme} />}
        </div>
...
// ======================== 配对页面 ========================
function PairingScreen({ appState, inviteCode, onCreatePair, onJoinPair, theme }) {
...
// ======================== 首页仪表盘 ========================
function DashboardView({ diaryData, myRole, moods, countdowns, anniversary, theme }) {
  ...
  const calcDays = (d) => Math.ceil((new Date(d) - new Date()) / 864e5);
  const calcAnnivDays = () => anniversary ? Math.floor(Math.abs(new Date() - new Date(anniversary)) / 864e5) : 0;
  ...
  const handleComfort = async () => {
    try { await api('/comfort'); } catch (e) { console.error(e); }
  };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, paddingTop: 8 }}>
      {/* 状态卡片 */}
      <div style={{ background: 'white', borderRadius: 32, padding: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', border: '4px solid white', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
        <div style={{ display: 'flex', gap: 16, zIndex: 10 }}>
          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div onClick={() => setIsMoodOpen(!isMoodOpen)} style={{ width: 52, height: 52, background: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, cursor: 'pointer', border: `3px solid ${theme.mainColorLight}`, boxShadow: '0 2px 6px rgba(0,0,0,0.06)', zIndex: 20 }}>
              {MOOD_OPTIONS.find(m => m.value === myMood)?.emoji}
            </div>
            <span style={{ fontSize: 10, marginTop: 6, fontWeight: 700, color: theme.mainColor, background: theme.mainColorSoft, padding: '2px 8px', borderRadius: 6 }}>我</span>
            {isMoodOpen && (
              <>
                <div style={{ position: 'fixed', inset: 0, zIndex: 20 }} onClick={() => setIsMoodOpen(false)} />
                <div style={{ position: 'absolute', top: 60, left: -8, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(12px)', borderRadius: 999, padding: '8px 12px', display: 'flex', gap: 12, zIndex: 30, boxShadow: '0 8px 24px rgba(0,0,0,0.1)', border: `2px solid ${theme.mainColorLight}` }}>
                  {MOOD_OPTIONS.map(m => <button key={m.value} onClick={() => handleMoodChange(m.value)} style={{ fontSize: 24, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>{m.emoji}</button>)}
                </div>
              </>
            )}
          </div>
          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: 52, height: 52, background: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, border: partnerNeedsComfort ? '3px solid #c7d2fe' : '3px solid #f1f5f9', animation: partnerNeedsComfort ? 'pulse 2s infinite' : undefined }}>
              {MOOD_OPTIONS.find(m => m.value === partnerMood)?.emoji}
            </div>
            <span style={{ fontSize: 10, color: '#94a3b8', marginTop: 6, fontWeight: 700, background: '#f8fafc', padding: '2px 8px', borderRadius: 6 }}>Ta</span>
            {partnerNeedsComfort && (
              <button onClick={handleComfort} style={{ position: 'absolute', top: -4, right: -8, background: theme.mainColor, color: 'white', padding: 6, borderRadius: '50%', border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 20 }}>
                <HeartHandshake size={14} />
              </button>
            )}
          </div>
        </div>
        <div style={{ textAlign: 'right', background: 'rgba(255,255,255,0.8)', padding: 10, borderRadius: 16, border: `1px solid ${theme.mainColorLight}` }}>
          <span style={{ fontSize: 9, color: '#94a3b8', fontWeight: 700, letterSpacing: 2 }}>在一起已经</span>
          <div><span style={{ fontSize: 24, fontWeight: 900, color: theme.mainColor }}>{calcAnnivDays()}</span> <span style={{ fontSize: 10, color: theme.mainColorMuted }}>天</span></div>
        </div>
      </div>

      {countdowns.length > 0 && (
        <div>
          <h3 style={{ fontSize: 11, fontWeight: 900, color: '#64748b', marginBottom: 12, marginLeft: 8, display: 'flex', alignItems: 'center' }}><Clock size={14} style={{ marginRight: 4, color: theme.mainColor }} /> 倒数日小星星 ✨</h3>
          <div style={{ display: 'flex', overflowX: 'auto', gap: 12, paddingBottom: 8, scrollbarWidth: 'none' }}>
            {countdowns.map(cd => {
              const dl = calcDays(cd.date); const past = dl < 0;
              return (
                <div key={cd.id} style={{ flexShrink: 0, width: 144, padding: 16, borderRadius: 24, border: `2px solid ${past ? '#f1f5f9' : theme.mainColorLight}`, background: past ? '#f9fafb' : 'white' }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 4 }}>{cd.title}</div>
                  <div><span style={{ fontSize: 24, fontWeight: 900, color: past ? '#d1d5db' : theme.mainColor }}>{Math.abs(dl)}</span> <span style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8' }}>{past ? '天前' : '天后'}</span></div>
                </div>
              );
            })}
          </div>
      ...
      <CalendarWidget diaryData={diaryData} myRole={myRole} theme={theme} />
    </div>
  );
}

// ======================== 日历 ========================
function CalendarWidget({ diaryData, myRole, theme }) {
  ...
  const saveDiary = async () => {
    ...
    setSaving(true);
    try { await api('/diary/save', { date: selectedDate, content: inputText }); setIsModalOpen(false); }
  ...
  return (
    <>
      <div style={{ background: 'white', borderRadius: 32, border: '4px solid white', padding: 20, boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {!isCurrentMonth && <button onClick={() => { setViewYear(today.getFullYear()); setViewMonth(today.getMonth()); }} style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 999, border: 'none', cursor: 'pointer', color: theme.mainColor, background: theme.mainColorSoft }}>回今天</button>}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '20px 8px', textAlign: 'center' }}>
...
// ======================== 心愿单 ========================
function WishlistView({ wishlist, myRole, theme }) {
  const pending = wishlist.filter(w => !w.completed);
  const completed = wishlist.filter(w => w.completed);

  const toggle = async (id) => { try { await api('/wish/toggle', { wishId: id }); } catch (e) { console.error(e); } };
  const del = async (id) => { try { await api('/wish/delete', { wishId: id }); } catch (e) { console.error(e); } };
          ...
          {pending.map(item => (
            <div key={item.id} onClick={() => toggle(item.id)} style={{ background: 'white', padding: 16, borderRadius: 24, border: '3px solid white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: '#334155' }}>{item.text}</span>
              </div>
            ...
            <div style={{ paddingTop: 24 }}>
              ...
              {completed.map(item => (
                <div key={item.id} onClick={() => toggle(item.id)} style={{ background: 'rgba(248,250,252,0.5)', padding: 16, borderRadius: 24, border: '2px solid #f1f5f9', display: 'flex', alignItems: 'center', cursor: 'pointer', marginBottom: 12, opacity: 0.7 }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#d1fae5', color: '#34d399', marginRight: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><CheckCircle2 size={16} strokeWidth={3} /></div>
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#94a3b8', textDecoration: 'line-through' }}>{item.text}</span>
                </div>
              ))}
            </div>
          )}
...
// ======================== 卡包 ========================
function VoucherView({ vouchers, myRole, theme }) {
  const [sub, setSub] = useState('available');
  const filtered = vouchers.filter(v => sub === 'available' ? v.status !== 'used' : v.status === 'used');
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#94a3b8', marginTop: 64, fontSize: 12, fontWeight: 700, background: 'white', padding: 48, borderRadius: 32, border: `4px dashed ${theme.mainColorLight}` }}>
            ...
            {sub === 'available' ? '还没有可用的免死金牌哦 🧸' : '还没有核销过的卡券'}
          </div>
        ) : filtered.map(v => <VoucherCard key={v.id} voucher={v} myRole={myRole} theme={theme} />)}
      </div>
    </div>
  );
}

function VoucherCard({ voucher, myRole, theme }) {
  const [flipped, setFlipped] = useState(false);
  const [stamping, setStamping] = useState(false);
  const isSender = voucher.senderRole === myRole;

  const handleApply = async (e) => { e.stopPropagation(); try { await api('/voucher/update', { voucherId: voucher.id, status: 'pending' }); } catch (e) { console.error(e); } };
  const handleStamp = async (e) => {
    e.stopPropagation(); setStamping(true);
    try { await api('/voucher/update', { voucherId: voucher.id, status: 'used' }); } catch (e) { console.error(e); }
  ...
  const backActions = () => {
    ...
    if (isSender) {
      ...
      return <div style={{ marginTop: 16, textAlign: 'center', fontSize: 10, color: '#94a3b8', background: '#f8fafc', padding: 8, borderRadius: 12, border: '2px dashed #f1f5f9' }}>等待Ta使用... 🤫</div>;
    } else {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: 190, cursor: 'pointer', perspective: 1000 }} onClick={() => setFlipped(!flipped)}>
      <div style={{ width: '100%', height: '100%', transition: 'transform 0.7s ease-out', transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0)' }}>
        <div style={{ position: 'absolute', inset: 0, borderRadius: 32, background: 'white', backfaceVisibility: 'hidden', display: 'flex', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <div style={{ width: '100%', height: '100%', borderRadius: 32, margin: 8, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', border: `3px dashed ${theme.mainColorMuted}`, background: `linear-gradient(135deg, ${theme.mainColorSoft}, white)` }}>
        ...
        <div style={{ position: 'absolute', inset: 0, borderRadius: 32, background: 'white', backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', border: `4px solid ${voucher.status === 'used' ? '#f1f5f9' : theme.mainColorLight}`, padding: 20, display: 'flex', flexDirection: 'column', opacity: voucher.status === 'used' ? 0.9 : 1, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
