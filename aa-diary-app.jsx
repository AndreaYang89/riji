...
function VoucherCard({ voucher, myRole, theme, onUpdateStatus, onRevoke }) {
  ...
  // 未翻转：显示正面；翻转后：显示背面（不用 3D 翻转，改为淡入切换，避免 absolute 高度问题）
  if (!flipped) {
    return (
      <div style={{ position: 'relative', width: '100%', cursor: 'pointer' }} onClick={() => setFlipped(true)}>
        <div style={{ borderRadius: 32, background: 'white', display: 'flex', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', minHeight: 200 }}>
          <div style={{ width: '100%', borderRadius: 32, margin: 8, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', border: `3px dashed ${theme.mainColorMuted}`, background: `linear-gradient(135deg, ${theme.mainColorSoft}, white)`, padding: '24px 16px' }}>
        ...
        {voucher.status === 'pending' && <div style={{ position: 'absolute', top: -8, right: -8, background: '#fbbf24', color: 'white', fontSize: 10, fontWeight: 900, padding: '6px 12px', borderRadius: 999, border: '2px solid white', transform: 'rotate(12deg)', animation: 'pulse 2s infinite', zIndex: 5 }}>求放过 🥺</div>}
        {voucher.status === 'revoked' && <div style={{ position: 'absolute', top: -8, right: -8, background: '#ef4444', color: 'white', fontSize: 10, fontWeight: 900, padding: '6px 12px', borderRadius: 999, border: '2px solid white', transform: 'rotate(12deg)', zIndex: 5 }}>已撤回 🚫</div>}
      </div>
    );
  }

  return (
    <div style={{ position: 'relative', width: '100%', cursor: 'pointer' }} onClick={() => setFlipped(false)}>
      <div style={{ borderRadius: 32, background: 'white', border: `4px solid ${voucher.status === 'used' || voucher.status === 'revoked' ? '#f1f5f9' : theme.mainColorLight}`, padding: 20, display: 'flex', flexDirection: 'column', opacity: voucher.status === 'used' || voucher.status === 'revoked' ? 0.9 : 1, boxShadow: '0 4px 16px rgba(0,0,0,0.08)', minHeight: 200 }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <h4 style={{ fontSize: 11, fontWeight: 900, color: '#94a3b8', borderBottom: '2px dashed #f1f5f9', paddingBottom: 8 }}>给全世界最好的你 🧸</h4>
          <div style={{ marginTop: 12, borderRadius: 16, padding: 16, fontSize: 14, fontWeight: 700, color: '#475569', lineHeight: 1.6, background: theme.mainColorSoft, border: `1px solid ${theme.mainColorLight}` }}>"{voucher.reason}"</div>
          {backActions()}
        </div>
